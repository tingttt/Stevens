...
public class ChatHelper {
    ...
    public void startMessageSync() {
        ...
        Bundle data = new Bundle();
        data.putBoolean("REUSE_CHANNEL", true); // Signal to reuse channel for continuous sync
        ...
        Log.d(TAG, "Periodic sync enqueued with interval: " + SYNC_INTERVAL + " minutes");
    }
    ...
    public void stopMessageSync() {
        ...
        RestMethod restMethod = new RestMethod(context);
        restMethod.shutdown(); // Ensure channel is closed
        syncRequest = null; // Clear request for future syncs
        Log.d(TAG, "Periodic sync canceled and channel closed");
